...
LOG_DIR="${PWD}/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/setup-auditlog-$(date +%Y%m%d-%H%M%S).log"
...
if [ -f "bin/otelcontribcol_linux_amd64" ]; then
    ...
    echo "Final Redis readiness check before starting collector..."
    REDIS_READY_START=0
    for i in $(seq 1 30); do
        if docker exec redis redis-cli -h "$REDIS_HOST_FOR_WAIT" -p "$REDIS_PORT_FOR_WAIT" ping 2>/dev/null | grep -q "PONG"; then
            echo "Redis is ready for collector start."
            REDIS_READY_START=1
            break
        fi
        echo "Waiting for Redis before collector start... ($i/30)"
        sleep 2
    done
    if [ $REDIS_READY_START -eq 0 ]; then
        echo "Error: Redis not reachable before starting collector. Aborting."
        exit 1
    fi
    ...
    MSYS_NO_PATHCONV=1 docker run -d --name otel-collector \
        ...
        sh -c "apt-get update -qq >/dev/null 2>&1 && apt-get install -y -qq netcat-openbsd redis-tools >/dev/null 2>&1 && sleep 5 && for i in \$(seq 1 40); do if nc -z '$REDIS_HOST_FOR_WAIT' '$REDIS_PORT_FOR_WAIT' 2>/dev/null; then echo 'Redis port is open at $REDIS_ENDPOINT'; break; fi; echo \"Waiting for Redis port at $REDIS_ENDPOINT... (\$i/40)\"; sleep 1; done && for i in \$(seq 1 10); do if redis-cli -h '$REDIS_HOST_FOR_WAIT' -p '$REDIS_PORT_FOR_WAIT' ping 2>/dev/null | grep -q PONG; then echo 'Redis responded to PING at $REDIS_ENDPOINT'; break; fi; echo \"Waiting for Redis PING at $REDIS_ENDPOINT... (\$i/10)\"; sleep 2; done && if ! redis-cli -h '$REDIS_HOST_FOR_WAIT' -p '$REDIS_PORT_FOR_WAIT' ping 2>/dev/null | grep -q PONG; then echo 'Redis PING failed, aborting collector start'; exit 1; fi && /otelcontribcol --config /auditlog-config.yaml"
