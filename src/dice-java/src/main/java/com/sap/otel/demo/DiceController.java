...
/**
 * REST controller providing a /rolldice endpoint similar to the Go example. Uses OpenTelemetry for audit logging.
 */
@RestController
public class DiceController {
	...
	public String rollDice(@PathVariable(value = "player", required = false) String player) {
		...
		otelLogger.logRecordBuilder().setSeverity(Severity.INFO).setBody(msg)
				.setAttribute(AttributeKey.stringKey("result"), String.valueOf(roll)).setAttribute(AttributeKey.stringKey("AUDIT-USER"), user)
				.emit();
		...
		for (int i = 0; i < logMessagesPerRequest; i++) {
			otelLogger.logRecordBuilder().setSeverity(Severity.INFO)
					.setBody(String.format("dice: %d, user: %s - bulk log message #%d", roll, user, i + 1)).emit();
		}
